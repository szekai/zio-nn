...
object TensorOps:
  ...
  private def createFromFlat(flat: Array[Double], targetShape: Array[Long], op: String): Task[NDArray] =
    for
  ...
  def createDoubleNd(data: Array[Double], shape: Array[Long]): Task[NDArray] =
    ZIO.attemptBlocking {
      baseManager.create(data.map(_.toFloat), new ai.djl.ndarray.types.Shape(shape*))
    }

  /** Reshape tensor to new shape. Total elements must match. */
  def reshape(a: NDArray, newShape: Array[Long]): Task[NDArray] =
    ZIO.attemptBlocking {
      a.reshape(new ai.djl.ndarray.types.Shape(newShape*))
    }

  /** Remove dimension of size 1 at position dim. */
  def squeeze(a: NDArray, dim: Int): Task[NDArray] =
    ZIO.attemptBlocking {
      val s = a.getShape
      if dim < 0 || dim >= s.dimension() then
        throw new IllegalArgumentException(
          s"squeeze dim $dim out of bounds for shape [${(0 until s.dimension()).map(s.get).mkString(", ")}]")
      if s.get(dim) != 1 then
        throw new IllegalArgumentException(s"squeeze requires dim $dim to have size 1, got ${s.get(dim)}")
      a.squeeze(Array(dim))
    }

  /** Add dimension of size 1 at position dim. */
  def unsqueeze(a: NDArray, dim: Int): Task[NDArray] =
    ZIO.attemptBlocking {
      val s = a.getShape
      val targetDim = if dim < 0 then s.dimension() + dim + 1 else dim
      a.expandDims(targetDim)
    }
  ...
  def zeros(targetShape: Array[Long]): Task[NDArray] =
    for
  ...
  def ones(targetShape: Array[Long]): Task[NDArray] =
    for
  ...
  def fill(targetShape: Array[Long], value: Double): Task[NDArray] =
    for
  ...
  def eye(size: Long): Task[NDArray] =
  ...
  def abs(a: NDArray): Task[NDArray] =
    for
      values <- toDoubleArray(a)
      targetShape <- shape(a)
  ...
  def square(a: NDArray): Task[NDArray] =
    for
      values <- toDoubleArray(a)
      targetShape <- shape(a)
  ...
  def sign(a: NDArray): Task[NDArray] =
    for
      values <- toDoubleArray(a)
      targetShape <- shape(a)
  ...
  def log(a: NDArray): Task[NDArray] =
    for
      values <- toDoubleArray(a)
      _ <-
  ...
  def sigmoid(a: NDArray): Task[NDArray] =
    for
      values <- toDoubleArray(a)
      targetShape <- shape(a)
  ...
  def maximum(a: NDArray, b: NDArray): Task[NDArray] =
    for
      leftShape  <- shape(a)
      rightShape <- shape(b)
  ...
  def diagonal(a: NDArray): Task[NDArray] =
    for
      targetShape <- shape(a)
      out <-
  ...
  def get(a: NDArray, indices: Array[Long]): Task[Double] =
    for
      targetShape <- shape(a)
      values      <- toDoubleArray(a)
  ...
  def slice(a: NDArray, start: Array[Long], end: Array[Long]): Task[NDArray] =
    for
      targetShape <- shape(a)
      values      <- toDoubleArray(a)
      out <- targetShape.length match
  ...
  def concatenate(a: NDArray, b: NDArray, axis: Int): Task[NDArray] =
    for
      leftShape  <- shape(a)
      rightShape <- shape(b)
      left       <- toDoubleArray(a)
  ...
  def gather(a: NDArray, indices: Array[Int]): Task[NDArray] =
    for
      targetShape <- shape(a)
  ...
  def lessThanOrEqual(a: NDArray, b: NDArray): Task[NDArray] =
    for
      leftShape  <- shape(a)
      rightShape <- shape(b)
  ...
  def unique(a: NDArray): Task[NDArray] =
    for
      targetShape <- shape(a)
  ...
  def countZeros(a: NDArray, threshold: Double = 1e-6): Task[Int] =
    toDoubleArray(a).map(_.count(v => math.abs(v) < threshold))

  def std(a: NDArray): Task[NDArray] =
    for
      values <- toDoubleArray(a)
      out <-
  ...
  def norm(a: NDArray): Task[Double] =
    toDoubleArray(a).map(values => math.sqrt(values.map(v => v * v).sum))

  def copy(a: NDArray): Task[NDArray] =
    for
      values <- toDoubleArray(a)
      targetShape <- shape(a)
